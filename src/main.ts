...
import { coord_to_index, for_chunks_in_radius, g_uid, get_chunk, get_chunk_index, get_object, wcoord_is_valid } from "./world.js";
...
function generate_world(world_description: WorldDescription) {
    ...
    let lairs_of_soulfires: {x: number, y: number}[] = []
    ...
    for (let lair of lairs_of_soulfires) {
        for (let i = 0; i < 500; i++) {
            ...
            if (!wcoord_is_valid(world_description, x, y)) continue
...
increase_soul_quality_button.onclick = () => {
    if (cores < UPGRADE_QUALITY_OF_SOULS.base_price_cores) return
...
let cores_counter = document.getElementById("cores-counter")! as HTMLDivElement;
...
function open_breach(item: Breach) {
    ...
    for (let wave = 0; wave < player.breach_waves; wave ++) {
        ...
        for (let i = 0; i < 40 / (wave + 1); i++) {
            ...
            if (!wcoord_is_valid(world_description, a.x + x, a.y + y)) continue
